...
is(Text::Fy::Utils::_aconvert($t, 0, 0), q~Ab?????????????????CEIyUuW~."Y"     .q~??????~, 'aconvert([ pure ])');
...
is(Text::Fy::Utils::_aconvert($t, 2, 0), q~Ab?????????!cY????ACEIyUuW~."Y"     .q~??????~, 'aconvert([ brutal ])');
...
is(simplify($t), q~Ab?????????!cY????ACEIyUuW~."Y".q~??????~, 'simplify');
